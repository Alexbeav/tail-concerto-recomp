...
#
# Writes: dist/tailconcerto-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix tailconcerto \
  --exe-name Tail_Concerto \
  --omit-openbios \
  --display-name "Tail Concerto" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Tail Concerto (USA) disc" \
  --runtime-dir mods \
